...
/**
 * Bulk-import a single-deduction CSV file into standing employee deductions
 * with installment terms. Independent of {@see EmployeeDeductionService}: it
 * owns its persistence and writes directly through the repositories so a large
 * file import never routes through the interactive CRUD path.
 *
 ...
 *   ...
 *
 * The `Code` cell resolves the deduction, `Date` the effective month/year, and
 * each data row an employee's per-month amount, total terms, and terms already
 * paid. Amounts feed `employee_deductions`; the term counts feed
 * `employee_deduction_terms`. Persistence is chunked so files with thousands of
 * employees commit in bounded batches instead of one long-held transaction.
 */
class BulkEmployeeDeductionService
{
    /**
     ...
     *
     * @return array{
     *     deduction: array{id: int, code: string, name: string}|null,
     *     period: array{month: int|null, year: int|null},
     *     rows: array<int, array<string, mixed>>,
     *     errors: array<int, array<string, mixed>>
     * }
     */
    public function parse(string $contents, ?int $deductionId = null): array
    {
        $records = iterator_to_array(Reader::createFromString($contents)->getRecords(), false);

        $deduction = $this->resolveDeduction($records[0][1] ?? null, $deductionId);
        ...
        if ($deduction === null) {
            $errors[] = ['error' => 'unknown_deduction_code', 'value' => trim((string) ($records[0][1] ?? ''))];
        }
        ...
        return [
            'deduction' => $deduction ? ['id' => $deduction->id, 'code' => $deduction->code, 'name' => $deduction->name] : null,
    ...
    /**
     * Parse then bulk-persist standing deductions + terms in chunked batches.
     ...
     * }
     */
    public function import(string $contents, ?int $deductionId = null, ?int $payrollPeriodId = null): array
    {
        $parsed = $this->parse($contents, $deductionId);
        ...
        foreach ($batches as $batch) {
            $stored += $this->storeBatch($batch, $parsed['deduction']['id'], $effectiveDate, $payrollPeriodId);
        }
     ...
     */
    private function storeBatch(array $rows, int $deductionId, Carbon $effectiveDate, ?int $payrollPeriodId): int
    {
        return DB::transaction(function () use ($rows, $deductionId, $effectiveDate, $payrollPeriodId): int {
            $logs = [];
            ...
            foreach ($rows as $row) {
                ...
                $deduction = $this->deductions->updateOrCreateStanding(
                    (int) $row['employee_id'],
                    $deductionId,
                    [
                ...
                $logs[] = [
                    'employee_deduction_id' => $deduction->id,
                    'action_by_id' => null,
                    'action' => $deduction->wasRecentlyCreated ? 'imported' : 'updated',
                    'remarks' => null,
            ...
            if ($logs !== []) {
                EmployeeDeductionLog::insert($logs);
            }
    ...
    /**
     * Resolve the deduction from an explicit id, else the file's `Code` cell
     * (exact code, then a fuzzy code/name match — e.g. "TAX" -> "WTAX").
     */
    private function resolveDeduction(?string $label, ?int $deductionId): ?Deduction
    {
        if ($deductionId !== null) {
            return Deduction::find($deductionId);
        }
        ...
        return Deduction::where('code', $label)->first()
            ?? Deduction::where('code', 'like', "%{$label}%")->first()
            ?? Deduction::where('name', 'like', "%{$label}%")->first();
    }
