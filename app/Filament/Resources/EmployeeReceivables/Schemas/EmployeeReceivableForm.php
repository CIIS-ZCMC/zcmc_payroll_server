...
namespace App\Filament\Resources\EmployeeReceivables\Schemas;
        ...
        return $schema
            ->components([
                ...
                Select::make('payroll_period_id')
                    ...
                    ->searchable()
                    ->helperText('Optional. Leave blank for an open-ended standing receivable.'),
                ...
                Section::make('Terms & balance')
                    ->schema([
                        Placeholder::make('terms_total')
                            ->label('Total terms')
                            ->content(fn (?EmployeeReceivable $record): string => (string) ($record?->terms?->total_terms ?? '—')),
                        Placeholder::make('terms_paid')
                            ->label('Paid terms')
                            ->content(fn (?EmployeeReceivable $record): string => (string) ($record?->terms?->paid_terms ?? '—')),
                        Placeholder::make('terms_amount')
                            ->label('Term amount')
                            ->content(fn (?EmployeeReceivable $record): string => static::money($record?->terms?->term_amount)),
                        Placeholder::make('terms_total_amount')
                            ->label('Total amount')
                            ->content(fn (?EmployeeReceivable $record): string => static::money($record?->terms?->total_amount)),
                        Placeholder::make('terms_remaining')
                            ->label('Remaining balance')
                            ->content(fn (?EmployeeReceivable $record): string => static::money($record?->terms?->remaining_balance)),
                        Placeholder::make('terms_completed_at')
                            ->label('Completed at')
                            ->content(fn (?EmployeeReceivable $record): string => (string) ($record?->terms?->completed_at ?? '—')),
                    ])
                    ->columns(3)
                    ->hidden(fn (?EmployeeReceivable $record): bool => $record === null),
            ]);
    }
